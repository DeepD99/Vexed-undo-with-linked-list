...
void Board::getUserInputAndMakeMove( char &moveAction)
{
    ...
    while( true) {
        ...
        cout << endl << moveNumber+1 << ". Your move: ";
        cin >> rowLetter;
...
//Here I create the class node with the appropriate declarations.
class Node
{
    public:
        Node *pNext;
...
//In this function im printing the linked list
//it prints the board followed by a : and then the move number
//looks through the linked list and keeps printing until it hits a NULL
void displayList(Node *pHead)
{
  cout << "   Score: " << pHead->score << ", List of board: moveNumber is: ";
  while(pHead != NULL){
    cout << pHead->theBoard.getBoardNumber() << ":" << pHead->theBoard.getMoveNumber();
    if(pHead->pNext != NULL){
      cout << "->";
    } 
    pHead = pHead->pNext;
...
//
void deleteNodeFromList(Node * &pHead, Board &theBoard, int &score, int &currentBoardIndex){
...
//What I'm doing here is that I'm adding the respective information into the list but
// its going to the front and not adding to the end
void addNodeToList(Node * &pHead, Board &theBoard, int score, int currentBoardIndex){
    Node *pTemp = new Node;
...
int main()
{
    ...
    addNodeToList(pHead,theBoard,score,currentBoardIndex);
    displayDirections();
        ...
        while( ! theBoard.boardIsClear() ) {
            ...
            if( moveAction == 'U') {
                ...
                // "*** You cannot undo past the beginning of the game.  Please retry. ***"
                  ...
                  deleteNodeFromList(pHead, theBoard, score, currentBoardIndex);
                  theBoard.displayBoard();
                }
